...
Taken from google recipees:
http://appengine-cookbook.appspot.com/recipe/a-simple-cookie-class/

Sample code:
class MainHandler(webapp.RequestHandler):
  def get(self):
    cookies = Cookies(self,max_age=180) #Optional arguments: max_age, path, domain, secure, httponly, version, comment
    #Expires will be calculated based on max_age. If secure is not set, it will be set to current connection status
    cookies['cookiename'] = value #Write a cookie to response
    result = cookies['cookiename'] #Read a cookie from request or pending request
